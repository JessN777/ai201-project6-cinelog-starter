...
def add_to_watchlist(user_id, film_id, public=True):
    ...
    Args:
        ...
        public (bool): Whether this watchlist entry is visible to other
            users. Defaults to True — see pr-response.md (Comment 4) for
            the reasoning behind this default. Callers that want a
            private-by-default entry can pass public=False explicitly.
...
def remove_from_watchlist(user_id, film_id):
    ...
    Follows the same lookup-then-delete pattern as remove_from_collection().
...
def get_watchlist(user_id):
    ...
    (most recently added first) to match get_collection()'s ordering.
    See pr-response.md (Comment 5) for the reasoning.
