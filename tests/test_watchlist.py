...
patterns used in tests/test_collection.py.
"""
...
def test_add_to_watchlist_defaults_to_public(app, sample_user, sample_film):
    """
    An entry created without an explicit `public` argument should default
    to public=True (see pr-response.md, Comment 4, for the reasoning).
    """
    with app.app_context():
        entry = add_to_watchlist(user_id=sample_user, film_id=sample_film)
...
def test_add_to_watchlist_respects_explicit_public_false(app, sample_user, sample_film):
    ...
    This is the edge case chosen for the "second test" stretch goal: it
    guards the visibility toggle (Comment 4 / stretch feature) rather than
    just the default path, since a caller-supplied override is the most
    likely way this behavior could silently regress.
    """
    with app.app_context():
...
def test_remove_from_watchlist_not_present_raises(app, sample_user, sample_film):
    ...
    """
    from services.watchlist_service import NotInWatchlistError
